...
meshv:SCR_Anatomy | meshv:SupplementaryConceptRecord | Anatomy, for example, rhombic lips. | ```SupplementalRecord @SCRClass="6"```
...
meshv:SCR_Organism | meshv:SupplementaryConceptRecord | Organisms, for example, SARS-CoV-2 variants. | ```SupplementalRecord @SCRClass="4"```
meshv:SCR_Population | meshv:SupplementaryConceptRecord | Population Group, for example, Hmong People. | ```SupplementalRecord @SCRClass="5"```
...
meshv:SupplementaryConceptRecord* | N/A | Supplementary Concepts are created in MeSH to aid in searching concepts that are not represented as descriptors. meshv:SupplementaryConceptRecord has six sub-classes: meshv:SCR_Anatomy, meshv:SCR_Chemical, meshv:SCR_Disease, meshv:SCR_Organism, meshv:SCR_Population, and meshv:SCR_Protocol. | ```SupplementalRecord```
