...
The vocabulary, `meta/vocabulary.ttl`, includes predicates used to indicate
which entities are still present in MeSH XmL, and which were present at one
time.  However, this meshrdf repository does not contain the data processing
logic used to maintain IRIs for obsolete/inactive entities.    Instead, the
N-triples files generated by these scripts are inputs to that process.

Once the data processing logic used to maintain IRIs is released, you can
